...
\begin{description}
  \item[charset] \hfill \\
	...
	\item[max\_errors] \hfill \\
		...
		Warnings do not interrupt the import process, data can be saved in LOVD even when warnings are displayed. 
\end{description}
...
diseaseid and individualid in section phenotypes
individualid in section screenings
...
On the import page the section import options has a dropdown menu for import mode. 
...
\begin{wrapfigure}[3]{r}{8cm} % Only wrap for 3 lines
  ...
  \end{leftbar}
\end{wrapfigure}
The option ``add only'' on the import page will inserted all data in the import file as new,
...
In general you can say, when you can modify certain data via the LOVD views,
 you can modify them via the update import as well.
Furthermore you can only modify one field per line.

For sections columns, genes, transcripts, diseases, individuals, phenotypes, screenings and 
 variants on genome you cannot modify the created\_by, created\_date, edited\_by, edited\_date fields.
...
The id's in the import file of each section is used to identify the records in the LOVD database.
Therefore you cannot modify the id fields in your import file.
To make sure your id's in the lovd database correspond with the id's in your import file,
 it is recommended to download a complete data set, custom columns, own data set or a gene's data set,
 depending on what you would like to update.

On the import page select the ``Update existing data'' option if you wish to modify data in your LOVD database.
You can use the simulate option to check your file for errors.
