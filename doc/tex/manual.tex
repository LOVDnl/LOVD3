...
\begin{tabular}{>{\bfseries}p{2.3cm} p{2.3cm} p{11.1cm}}
  ...
  space & AM DA & Results where both `AM' and `DA' are occuring.\\ \hline
  | & AM|DA & Results where either `AM' or `DA' occurs.\\ \hline
  \^{} & \^{}AM & Results which start with `AM' (prefix search).\\ \hline % I don't like \textasciicircum in this case.
  \$ & AM\$ & Results which end with `AM' (suffix search).\\ \hline
  ="" & ="AM" & Results which exactly match `AM' (exact search).\\ \hline
  ! & !AM & Results which do not contain `AM'.\\ \hline
  !="" & !="AM" & Results which do not exactly match `AM'.\\ \hline
  > & >23 & Values higher than 23 (numeric and date columns).\\ \hline
  < & <23 & Values lower than 23 (numeric and date columns).\\ \hline
\end{tabular}
\vskip \baselineskip
