...
\date{2014-11-26} % I guess it's easier to use this as a "Last modified" column. %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
For more information on the different column types, see the \hyperlink{sec:custom_column_categories}
...
\begin{wrapfigure}[3]{r}{8cm} % Only wrap for 3 lines
  ...
  \end{leftbar}
\end{wrapfigure}
LOVD supports exporting the custom data col\-umns, including the columns' system-wide settings, to \hyperlink{sec:download_import_format}{\nameref{sec:download_import_format}}.
...
\begin{wrapfigure}[3]{r}{8cm} % Only wrap for 3 lines
  ...
  \end{leftbar}
\end{wrapfigure}
Custom columns downloaded in the \hyperlink{sec:download_import_format}{\nameref{sec:download_import_format}} 
 can be reimported into LOVD.
...
Select ``I want to add phenotype information to this individual'' (figure 
 \hyperlink{fig:submission_of_individual}{\ref{fig:submission_of_individual}}), fill all required field and select 
...
If you wish to add phenotype information while this option is disabled, you have to edit the individual, see
 section ``\hyperlink{ssec:add_phenotype}{\nameref{ssec:add_phenotype}}'' for how to. 
...
\begin{figure}[ht]
  ...
  \caption{%
    ...
    Note: Submitter do not have the option ``I want to upload a file with genomic variant data'' available.}
  \end{shaded}
\end{figure} 
...
although the option ``Back to individual'' is not available when you submit a screening to an existing  individual.
...
The first line of the file contains the file's header line, which indicates the LOVD version number as well as the type of file.
The LOVD version number is encoded and provided by LOVD when creating a download.
When importing, LOVD can decode the version and in case the download format has changed between versions, convert the data into the correct format.
...
\begin{description}
  \item[Columns] \hfill \\
    ...
    Most records in this section are created during installation of LOVD. 
	Columns in the import file that already exist in the database are skipped during import. 
	When this happens a warning is displayed, but the import will continue.
  \item[Genes] \hfill \\
    Genes, including external IDs. \emph{Can currently not yet be imported.}
  	Genes can  be mentioned in the file, if so, they should already be present in the database before
  	 importing the entire file.
	If mentioned in the import file, the genes should be created manually in the database.
  \item[Transcripts] \hfill \\
    Transcript variants, including external IDs. \emph{Can currently not yet be imported.} 
	Transcripts can be mentioned in the file, if so, they should already be present in the database before
   	 importing the entire file.
  	If mentioned, the transcript should be created manually in the database.
  \item[Diseases] \hfill \\
    General disease information.
 	LOVD can import new diseases, but it does not accept diseases with the same name or id\_omim, this will
   	 generate an warning, but the import will continue.
  ...
  \item[Variants\_On\_Genome]* \hfill \\
    ...
    When you want to link an individual to a variant you can do that with sections Screenings and
     Screenings\_To\_Variants.
    Individuals are linked with section Screenings via the individualid field in the Screenings section.
    In section Screenings\_To\_Variants you can link screenings and variants. 
...
\end{description}
\noindent
Categories indicated with an asterisk (*) are data types with custom columns (also see ``\hyperlink{sec:custom_column_categories}{\nameref{sec:custom_column_categories}}''),
...
\begin{description}
  \item[charset] \hfill \\
    ...
    During import, usually the file encoding is detected given the file's contents, in which case this setting is ignored.
\end{description}
...
When importing a file, being able to change this line allows you to skip certain columns or decide yourself in which order you would like to display the columns in the file.
To clearly separate the line with column names from the data lines, the column names are surrounded by two curly braces on each side (\{\{ and \}\}).
Field names can not contain spaces, match the field's name in the database exactly, and the field names are separated by exactly one tab character from each other.
\vskip \baselineskip

The section's data is also in tab-delimited format.
Each field is preferably enclosed by double quotes.
All existing quotes in the data, are escaped by the backslash character (\textbackslash), so \texttt{\symbol{34}} becomes \texttt{\textbackslash\symbol{34}}.
Other special characters are \texttt{\textbackslash{}t} for a tab character, and \texttt{\textbackslash{}r\textbackslash{}n} for a linebreak.
...
Others are displayed, but for the user appear as selection lists with human-readable values, whereas in the database only numbers are stored.
To aid in understanding these fields and their values, all fields that may cause confusion are listed here.

Custom columns are not listed here, since every LOVD installation has different columns enabled and custom columns provide explanations about their use by themselves.
...
%  \item[id\_mutalyzer] (from section Transcripts)\hfill \\
  ...
  \end{tabular}
  \vskip \baselineskip
  Individuals, Phenotypes and Variants\_On\_Genome with status ``In progress'' or ``Pending'', respectively   	 value 1 and 2, can not be imported.
  The manager has 3 options:
  \begin{enumerate}
    \item Process the individual in the original LOVD installation and download the file again.
    \item Change the statusid value to a valid value.
    \item Exclude this record for import by removing it from the import file or comment the record with a \#.
    If you do this you have to make sure that all related data from other sections are excluded as well.
    Else LOVD returns a error \textit{``...does not exist in the database and is not defined in this import 	 file.''}
  \end{enumerate}
...
\subsection{Importing the complete data set to LOVD}
...
See section ``\hyperlink{sec:download_import_format}{\nameref{sec:download_import_format}}'' about import details
 on each section.
...
\begin{wrapfigure}[3]{r}{8cm} % Only wrap for 3 lines
  ...
  \end{leftbar}
\end{wrapfigure}
Managers can download a data file with only the custom columns witch are added to an LOVD installation.
...
\subsection{Importing a custom columns data set to LOVD}
\begin{wrapfigure}[3]{r}{8cm} % Only wrap for 3 lines
  \vspace{-25pt}
  \begin{leftbar}
    Required level: Manager\\
  ...
  \end{leftbar}
\end{wrapfigure}
The custom column data set downloaded by a manager can be uploaded to an LOVD.
Most of the records in this section are default custom columns.
Columns which already exists in the database are not imported, these columns are skipped during import.
...
\subsection{Importing own data set to LOVD}
\begin{wrapfigure}[3]{r}{8cm} % Only wrap for 3 lines
  \vspace{-25pt}
  \begin{leftbar}
    Required level: Manager\\
  ...
  \end{leftbar}
\end{wrapfigure}
The own data set downloaded by a submitter can not be uploaded to an LOVD without modifications.
Some sections must be modified and as with all import files, only a manager can import the data file.
 
Before import the header name must be changed. 
Replace ``Owned data download'' with ``Full data download''.

If the diseases in the import file do not exist in LOVD, the these diseases will be added during the import.
These new disease will not have a link with any gene, to create a link you have to edit the gene.
See ``\hyperlink{ssec:relation_to_disease}{Relation to diseases}'' for how to create a link between a gene and
 a disease.

For more import details on each section, see section ``\hyperlink{sec:download_import_format}
 {\nameref{sec:download_import_format}}''.
...
\subsection{Importing a gene's data set to LOVD}
\begin{wrapfigure}[3]{r}{8cm} % Only wrap for 3 lines
  \vspace{-25pt}
  \begin{leftbar}
    Required level: Manager\\
  ...
  \end{leftbar}
\end{wrapfigure}
The gene data set downloaded by a manager can be uploaded to an LOVD.
