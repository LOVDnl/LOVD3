...
LOVD is designed to provide a flexible, freely available tool for gene-centered collection and display of DNA variants.
...
Also new in 3.0 is full Next Generation Sequencing (NGS) support,
 with the ability to import VCF files.
...
To search for prefixes or suffixes, see the table further below.
...
LOVD allows boolean search terms, meaning you can construct complex queries with
 \texttt{AND}, \texttt{OR} and \texttt{NOT} logic.
Typing a space between search terms is treated as \texttt{AND}.
\texttt{OR} is indicated with a pipe `|' with no spaces around it and
 \texttt{NOT} is indicated by an exclamation mark `!',
...
\begin{tabular}{>{\bfseries}p{2.5cm} p{2.6cm} p{10.6cm}}
  ...
  | & Arg|Ser & Results containing `Arg' or `Ser'.\\ \hline
  ! & !fs & Results not containing `fs'.\\ \hline
  \^{} & \^{}p.(Arg & Results beginning with `p.(Arg' (prefix search).\\ \hline % I didn't like \textasciicircum.
  \$ & Ser)\$ & Results ending with `Ser)' (suffix search).\\ \hline
  ="" & ="" & Results with this field empty.\\ \hline
  ="" & ="p.0" & Results exactly matching `p.0' (exact search).\\ \hline
  !="" & !="" & Results with this field not empty.\\ \hline
  !="" & !="p.0" & Results not exactly matching `p.0'.\\ \hline
  combination & *|Ter !fs & Results containing `*' or `Ter' but not containing `fs'.\\ \hline
  < & <2020 & Values before 2020 (date fields).\\ \hline
  <= & <=2020-06 & Values on or before June, 2020 (date fields).\\ \hline
  > & >2020-06 & Values after June, 2020 (date fields).\\ \hline
  >= & >=2020-06-15 & Values on or after June 15th, 2020 (date fields).\\ \hline
%  combination & 2019|2020 <2020-03 & Values in 2019 or 2020, and before March, 2020 (date fields).\\ \hline % Sorry, no space for this.
  combination & >2009 <2020 & Values from 2010 to 2019 (date fields).\\ \hline
  & 23 & Values exactly matching 23 (numeric fields).\\ \hline
  < & <23 & Values lower than 23 (numeric fields).\\ \hline
  <= & <=23 & Values lower than, or equal to, 23 (numeric fields).\\ \hline
  > & >23 & Values higher than 23 (numeric fields).\\ \hline
  >= & >=23 & Values higher than, or equal to, 23 (numeric fields).\\ \hline
  combination & >=20 <30 !23 & Values from 20 to 29, but not equal to 23 (numeric fields).\\ \hline
\end{tabular}
\vskip \baselineskip
...
\vskip 0.5\baselineskip % This page is too crammed but if I don't do this, everything below goes to hell.
\noindent
\begin{tabular}{>{\bfseries}p{5.3cm} p{10.8cm}}
  Asian & Results containing `Asian', but also containing `Caucasian'.\\ \hline
  Asian !Caucasian & Results containing `Asian', but not containing `Caucasian'.\\ \hline
  Asian|African !Caucasian & Results containing `Asian' or `African', but not `Caucasian'.\\ \hline
  "South Asian" & Results containing `South Asian', but not `South East Asian'.\\ \hline
\end{tabular}
...
\section{Transcripts}
\begin{wrapfigure}[5]{r}{7cm} % Wrap for 5 lines.
  \vspace{-25pt}
  \begin{shaded}
...
\begin{description}
  ...
  See section ``\nameref{sec:transcript_create}'' on how to add a new transcript to a gene.
...
\begin{wrapfigure}[16]{r}{7cm}
  ...
  \end{shaded}
\end{wrapfigure}
When no gene is selected, the default page of the variants tab shows all variants on the genome level.
When a gene is selected (shown in the page header),
 the default page of the variants tab shows all unique variants in the selected gene.

\begin{description}
  \item [View all variants] \hfill \\
  This view displays all variants in this LOVD installation on the genome level.
...
\begin{description}
  ...
  This menu item is only available for Managers or higher.
  See section ``\nameref{sec:enabling_custom_columns}'' on how to enable custom columns.
...
\end{description}
% You need an empty line here, otherwise LaTeX breaks. But I need to separate these lists to fix the crazy wrapping.
...
If you're experienced with web servers and you want to set it up yourself: LOVD has
...
LOVD will force the use of the UTF-8 character set,
...
You need a \emph{hostname}, \emph{username}, \emph{password} and a \emph{database name}
 to be able to install LOVD.
If you are installing LOVD on a remote server, be sure to have the
...
Please note that these are not the recommended settings for production environments,
 but it will help you quickly set up LOVD on your computer.
\vskip \baselineskip

Please go through the entire config.ini.php file to determine if you need to change any of the other settings.

\begin{warntable}
A .htaccess file is put in the root directory of your LOVD
...
Make sure you have the .htaccess file into your LOVD directory,
...
To install LOVD on a remote webserver,
 upload the LOVD directory with all the files to the webserver by, for instance, (s)FTP.
If you install LOVD on your own computer, you do not need that step.
\vskip \baselineskip

Next, point your web browser to the directory where you've saved the LOVD files.
This could, for instance, be \texttt{http://localhost/LOVDv.3.0/} or, if you're using a remote server,
...
Both the PHP and MySQL versions will be checked, to make sure your LOVD
...
If all looks well, LOVD will tell you all requirements are OK
 and you're ready to start installing LOVD.
\vskip \baselineskip
...
The database administrator will be the first user registered with LOVD
 and has full access to all of LOVD's functionalities.
...
The final form in the installation process is completing
 the initial configuration of the LOVD system settings.
...
The only two settings that cannot be changed at a later time, are the install lock, which is checked by default,
 and the human build that the system will use to map your variants to.
...
If everything was filled in correctly, LOVD will register
 the LOVD system settings.
...
Now that you're done installing LOVD, click the ``Continue to Setup area \guillemotright'' button
 to be forwarded to the \hyperlink{chap:setup}{LOVD setup area}.
...
\begin{description}
  ...
  The following information is sent: the number of submitters,
...
\begin{description}
  ...
  If a submitter, curator, or manager has lost their password, enabling this ``I forgot my password''
...
\subsubsection{Announcement details}
...
\subsection{Editing an announcement}
\begin{wrapfigure}[3]{r}{8cm} % Only wrap for 3 lines
  \vspace{-25pt}
  \begin{leftbar}
    Required level: Manager\\
...
\begin{wrapfigure}[3]{r}{8cm} % Only wrap for 3 lines
  ...
  \end{leftbar}
\end{wrapfigure}
Only the database administrator can uninstall LOVD,
 provided the uninstall lock has not been set during installation of LOVD.
The uninstall lock is set within the MySQL database and can only be removed by directly accessing MySQL.
If you know how to handle MySQL, you can also uninstall LOVD through MySQL directly by removing all LOVD tables,
...
\begin{infotable}
Please note that the LOVD uninstaller will not remove the LOVD files itself,
 just the database tables that it has created during install.
...
In LOVD 3.0, \emph{authorized users} are everyone who registered for an account at the LOVD installation,
 including the database administrator who created their account during the installation process.
...
Therefore, a curator only has access to the gene databases they are appointed to, and not to the system-wide setup area.
...
This part of the registration form contains the basic fields with which the user identifies themself.
\begin{description}
  \item[Name] \hfill \\
...
\begin{description}
  ...
  The user will not be able to work with LOVD until they have changed their password.
\end{description}
...
\begin{description}
  ...
  When enabled, the user's details will be emailed to the email address(es) given,
   informing the user immediately about their account details.
  The submitter registration form does not have this checkbox,
   but this feature is \emph{always} active for submitter registrations.
\end{description}
...
\section{Sharing access with other users}
\begin{wrapfigure}[3]{r}{8cm} % Only wrap for 3 lines
  \vspace{-25pt}
  \begin{leftbar}
    Required level: Submitter\\
  ...
  \end{leftbar}
\end{wrapfigure}
An authorized user can share access to their data with other users.
The other users can then see any non-public data entries owned by the sharing user,
 and if specified, they can also edit those entries.
Additionally, all non-public fields of all entries owned by the sharing user, become available to this user, too.
...
\begin{figure}[ht]
  \begin{shaded}
  \frame{\includegraphics[width=\linewidth]{c05s05_screenshot_access_sharing.png}}%
  \caption{%
...
Gene databases can also be created automatically for you, when you are importing VCF files in LOVD.
However, this chapter deals with manually creating gene databases.
%TODO
%For more information on how to import VCF files, see ...
...
\begin{description}
  ...
  Select the genomic reference sequence (NG or NC accession number).
...
If no gene is selected, clicking on the same tab shows all variants in the database on the genome level.
...
Unchecking this checkbox turns this user in a collaborator for this gene;
 they can still see all public and unpublic data in this gene database, but they can't edit it, like curators can.
If the user is to be a curator, they will receive notifications of variant data submissions and submission updates.
...
\begin{description}
  ...
  This will appear when a user moves their mouse over the column header in the data tables.
...
\begin{description}
  ...
  When the user moves their mouse over the question mark icon, the text entered here is shown.
...
All other fields on this form are explained in the section ``\nameref{sec:custom_column_create}''.
...
For more information on the format, please see section ``\nameref{sec:download_import_format}''.
...
When finishing a submission, the curator(s) will receive an email notification, inviting them to publish the data.
\vskip \baselineskip

This chapter describes adding data to LOVD using the web forms.
If you are a curator or up and you wish to add data to LOVD using a tab-delimited file upload,
 please see chapter ``\nameref{chap:download_n_import}''.
If you're a programmer and you would like to submit data in JSON format directly from your application
 using the LOVD3 submission API, please see chapter ``\nameref{chap:api}''.
...
Please see section ``\nameref{sec:submit_summary_variant_data}'' for details.
\vskip \baselineskip
...
For uploading genomic variant data files, the Variant Call Format (VCF) version 4.0 or higher is supported.
...
%  \item[allow\_count\_all] (from section Columns)\hfill \\
  ...
  The variant's effect on a protein's function.
  The first digit indicates the variant's effect as reported by the source.
  ...
  A score of 97 means a variant is reported to affect the function of the protein having strong proof, but the curator
...
See section ``\nameref{sec:download_import_format}'' for more details about the file format.
...
See section ``\nameref{sec:download_import_format}'' for details on each section.
...
See ``\nameref{ssec:relation_to_disease}'' for how to create a link between a gene and a disease.
...
The column section is missing in this download file, but apart from that, this is the same import file as the
 complete data set.
See section ``\nameref{ssec:importing_full_data}'' on details about importing a gene's data set.
...
\hypertarget{ssec:api_possibilities}{}
\subsection{Possibilities}
\label{ssec:api_possibilities}
...
Note that this API was introduced in LOVD 3.0-18, but was only fully functional from LOVD 3.0-21.
...
The JSON format allows for multiple individuals to be submitted at once;
 the file is required to contain at least one individual with each at least one variant.
The mandatory columns are the same as otherwise mandatory in LOVD3.
For fields that require a fixed set of options, such as the individual's gender,
 the possible options are mentioned in a comment in the example JSON file.
JSON officially can't contain comments so a JSON parser might fail on the example file, but LOVD3 is OK with them.
...
\subsection{Mandatory fields to set before submission}
Before you can submit your data using this example file, you'll have to change the values of some of the fields.
The `\texttt{lsdb}' element has an `\texttt{@id}' and an `\texttt{@uri}' field that needs to be set.
The `\texttt{@id}' is the LOVD's ID, which can be found by
 the database administrator in the API authorization token dialog (see ``\nameref{ssec:getting_your_auth_token}''.
If you do not know who the LOVD's administrator is, submit the file without a valid ID
 (see ``\nameref{ssec:submitting_the_json_file}'') and LOVD will respond with the administrator's contact information.
\vskip \baselineskip

The `\texttt{source}' element under `\texttt{lsdb}' also requires a few fields to be filled in.
For `\texttt{name}', fill in the name of the lab that is submitting the data.
% Note that the above field is never used, but it is a requirement from VarioML.
...
\begin{figure}[ht]
  \begin{shaded}
  \frame{\includegraphics[width=\linewidth]{screenshot_auth_token_created.png}}%
  \caption{%
...
\end{verbatim}
  \caption{%
    Example of how to submit a VarioML JSON file to the submission API, using \texttt{curl}.
    In this example, the JSON file is named \texttt{submission.json} and is located in the current working directory.}
    \label{fig:submission_api_curl_example}
  \end{shaded}
\end{figure}

The submission API will respond in JSON, sending its version, a messages array,
 a warnings array, an errors array, and a data array (see figure \ref{fig:submission_api_output_error}).
...
\end{verbatim}
  \caption{%
    Example submission API output, when sending ``\texttt{\{"lsdb":\{\}\}}''.
    All arrays shown are always present, but can be empty.}
    \label{fig:submission_api_output_error}
  \end{shaded}
\end{figure}

The `\texttt{messages}' array is for information of successful submissions.
The `\texttt{warnings}' array is for output of information you should review because it might be a potential problem,
 although it will not prevent a successful submission.
The `\texttt{errors}' array contain fatal errors.
The `\texttt{data}' array is reserved for returning data and not used in \texttt{POST} requests.
\vskip \baselineskip

A successful submission results in an LOVD3 import file being created on the server.
This file then needs to be imported into LOVD3.
In LOVD 3.0-21, submissions done through the API are not yet scheduled for import automatically,
 it requires manual intervention of a manager.
See the \texttt{INSTALL.txt} file that is shipped with LOVD for more information.



\subsection{Different versions of this API}
...
\chapter{Updating LOVD}
Several times a year, a new LOVD build is released.
An update may contain bug fixes, new features, new custom columns and such.
Therefore, each update has a priority setting to indicate how important it is
 to update your LOVD installation to the new release.
\vskip \baselineskip

Depending on whether or not LOVD has found that there is an update available,\
 an update icon appears at the bottom right-hand side of the screen:
\vskip \baselineskip
...
If you are logged in as an authorized user,
 you can click the update notification icon for more information on the status of the update.
If an update is available, detailed information is presented:

\begin{description}
  \item[Last checked for updates:]
  Shows the date LOVD last checked for a new update.
  \item[Latest version:]
  Shows the detected update.
  \item[Release date:]
  The date this update has been released.
  \item[Priority level:]
  Shows how important the LOVD developers consider this update.
  Can contain ``Optional'', ``Common'', ``Suggested'', ``Recommended'',
   ``\textcolor{red}{Important}'' or ``\textcolor{red}{\textbf{Critical}}'', with increasing priority.
  \item[Release info:]
...
\nameref{sec:name_of_label}

\label{marker}
you give the object you want to reference a marker, you can see it like a name. For pictures, it must appear after or inside the \caption.
\ref{marker}
...
\nameref{marker}
It will print the name of the assigned object.
\hyperref[marker]{text}
Then we don't need \hypertarget anymore!
...
Also, how can curators curate new entries? It's not explained anywhere...
