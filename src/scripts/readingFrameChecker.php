...
$sGene = htmlspecialchars($_REQUEST['gene']);
$sTranscript = htmlspecialchars($_REQUEST['transcript']);
...
if (is_readable($sFilePath)) {
    ...
    lovd_showInfoTable('There is no structure table available for <B>' . $sTranscript . '</B>. To get one, you\'ll have to have a GenBank file for your gene with genomic sequence, and run the <A href="scripts/refseq_parser.php">Reference Sequence Parser</A>.', 'warning');
    exit;
}
...
print('<P>Currently viewing gene/transcript: <B>' . $sGene . ' / ' . $sTranscript . '</B></P>' .
        '      <BR>' . "\n\n" .
