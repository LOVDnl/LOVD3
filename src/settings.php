...
if (PATH_COUNT == 1 && ACTION == 'edit') {
    ...
    if (!empty($_POST)) {
        ...
        if (!lovd_error()) {
            ...
            $aFields = array(
                'system_title', 'institute', 'location_url', 'email_address', 'send_admin_submissions', 'api_feed_history',
                'proxy_host', 'proxy_port', 'proxy_username', 'proxy_password',
                'md_apikey',
                'logo_uri',
                'send_stats', 'include_in_listing',
                'allow_submitter_registration', 'lock_users', 'allow_unlock_accounts', 'allow_submitter_mods', 'allow_count_hidden_entries', 'use_ssl', 'use_versioning');
