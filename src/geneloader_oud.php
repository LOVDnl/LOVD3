...
foreach ($aHGNCFile as $nLine => $sLine) { // T: Leest de HGNC file regel per regel uit
    ...
    //print_r($aGene['refseq_UD']);


    // Now load transcripts and see what we've got.
    //print($_CONF['mutalyzer_soap_url']. "\n"]);
    ...
    if ($aLine['gd_pub_refseq_ids']) {
        ...
        $ch=curl_init('https://mutalyzer.nl/json/getTranscriptsAndInfo?genomicReference=' . $aLine['gd_pub_refseq_ids'] . '&geneName='. $aLine['gd_app_sym']);
        ...
        //print_r(file(str_replace('/services', '', $_CONF['mutalyzer_soap_url']) . '/json/getTranscriptsAndInfo?genomicReference=' . $aLine['gd_pub_refseq_ids'] . '&geneName=' . $aLine['gd_app_sym']));
                ...
                if ($aAvailableTranscript['id']) { // Is this check needed? Copied from genes.php.
                    ...
                    $aTranscriptsInUD[$sIDWithoutVersion][$nVersion] =
                        array(
                            ...
                            //$aLine['gd_app_name'] => $aAvailableTranscript['proteinTranscript']['product'],
        ...
        //print($aTranscriptsInUD[$sIDWithoutVersion][$nVersion]);
    }
    //print($aTranscriptsInUD[$sIDWithoutVersion][$nVersion]);
