...
class FalseInsertImportTest extends LOVDSeleniumBaseTestCase
{
    public function testFalseInsertImport()
    {
        $this->open(ROOT_URL . "/src/import");
        $this->type("name=import", ROOT_PATH . "/tests/test_data_files/FalseInsertImport.txt");
        ...
        $this->assertTrue((bool)preg_match('/^[\s\S]*Error \(Variants_On_Genome, line 90\): The field \'position_g_start\' must contain a positive integer, "abc" does not match\.[\s\S]*$/', $this->getBodyText()));
        ...
        $this->assertTrue((bool)preg_match('/^[\s\S]*Error \(Variants_On_Transcripts, line 97\): The field \'position_c_start\' must contain an integer, "abc" does not match\.[\s\S]*$/', $this->getBodyText()));
        ...
        $this->assertTrue((bool)preg_match('/^[\s\S]*Error \(Variants_On_Transcripts, line 98\): The field \'position_c_start\' must contain an integer, "abc" does not match\.[\s\S]*$/', $this->getBodyText()));
