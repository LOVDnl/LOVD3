...
class DeleteGeneIVDTest extends LOVDSeleniumWebdriverBaseTestCase
{
    public function testDeleteGeneIVD()
    {
        ...
        $element->click();
        $this->assertEquals("You are about to delete 1 transcript(s) and related information on 3 variant(s) on those transcripts. Please fill in your password one more time to confirm the removal of gene IVD.", $this->getText(WebDriverBy::xpath("//*/table[@class='info'][2]")));
        $this->enterValue(WebDriverBy::name("password"), "test1234");
        $element = $this->driver->findElement(WebDriverBy::xpath("//input[@value='Delete gene information entry']"));
        $element->click();
