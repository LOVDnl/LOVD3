...
foreach ($aSQL as $sSQLInput => $sSQLExpectedOutput) {
    ...
    if (strpos($sSQLOutput, 'SQL_CALC_FOUND_ROWS') !== false) {
        ...
        $_DB->query($sSQLOutput);
        $nFoundOutput = $_DB->query('SELECT FOUND_ROWS()')->fetchColumn();
    } else {
        $nFoundOutput = $_DB->query($sSQLOutput)->fetchColumn();
    }
    $tSQLOutput = microtime(true) - $t;

    printf('Query %02d. Size %d/%d bytes. Time: %.2f%% (%.5f/%.5f). Results: %d/%d.' . "\n",
        $i, strlen($sSQLOutput), strlen($sSQLInput), ($tSQLOutput/$tSQLInput), $tSQLOutput*1000, $tSQLInput*1000, $nFoundOutput, $nFoundInput);
